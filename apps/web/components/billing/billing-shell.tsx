...
function Plans({ currentPlan, interval, setInterval, onChange }: { currentPlan: BillingPlanId; interval: BillingInterval; setInterval: (value: BillingInterval) => void; onChange: (plan: BillingPlanId) => void }) {
  return <div className="space-y-5"><SectionIntro eyebrow="Plans" title="Choose the right runway" body="Upgrade, downgrade, or switch billing intervals at any time. Changes are mocked until Stripe is connected." /><div className="inline-flex rounded-xl border border-border bg-surface p-1"><button className={`rounded-lg px-4 py-2 text-xs font-medium ${interval === "monthly" ? "bg-elevated text-foreground" : "text-secondary"}`} onClick={() => setInterval("monthly")}>Monthly</button><button className={`rounded-lg px-4 py-2 text-xs font-medium ${interval === "yearly" ? "bg-elevated text-foreground" : "text-secondary"}`} onClick={() => setInterval("yearly")}>Yearly <span className="ml-1 text-success">Save 17%</span></button></div><div className="grid gap-4 lg:grid-cols-3">{billingPlans.map((plan) => <Card key={plan.id} elevated={plan.popular} className={`flex flex-col ${plan.popular ? "border-primary/50" : ""}`}>{plan.popular ? <Badge variant="accent" className="mb-4 w-fit">Most popular</Badge> : null}<h3 className="text-xl font-semibold">{plan.name}</h3><p className="mt-2 min-h-10 text-sm leading-5 text-secondary">{plan.description}</p><p className="mt-6 text-3xl font-semibold">${interval === "monthly" ? plan.monthlyPrice : plan.yearlyPrice}<span className="text-xs font-normal text-secondary"> / {interval === "monthly" ? "mo" : "yr"}</span></p><ul className="mt-6 flex-1 space-y-3 border-t border-border pt-5">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm text-secondary"><Check className="h-4 w-4 shrink-0 text-success" />{feature}</li>)}</ul><Button className="mt-6 w-full" variant={currentPlan === plan.id ? "secondary" : plan.popular ? "primary" : "secondary"} disabled={currentPlan === plan.id} onClick={() => onChange(plan.id)}>{currentPlan === plan.id ? "Current plan" : plan.monthlyPrice > (billingPlans.find((item) => item.id === currentPlan)?.monthlyPrice ?? 0) ? "Upgrade" : "Downgrade"}</Button></Card>)}</div><Card><SectionHeading title="Feature comparison" /><Comparison /></Card></div>;
}
...
function Addons({ onBuy }: { onBuy: (name: string) => void }) {
  const addons = [{ name: "Audit Events", detail: "10,000 additional events", price: 12, icon: Activity }, { name: "AI Agents", detail: "5 additional agents", price: 19, icon: Sparkles }, { name: "Workspaces", detail: "1 additional workspace", price: 15, icon: Layers3 }, { name: "Admin Seats", detail: "3 additional admin seats", price: 24, icon: Users }];
  return <div className="space-y-5"><SectionIntro eyebrow="Credits & add-ons" title="Add capacity without changing plans" body="Buy one-time capacity boosts whenever your team needs them." /><div className="grid gap-4 sm:grid-cols-2">{addons.map(({ name, detail, price, icon: Icon }) => <Card key={name} className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Icon className="h-5 w-5" /></div><div className="min-w-0 flex-1"><h3 className="font-semibold">{name}</h3><p className="mt-1 text-sm text-secondary">{detail}</p><p className="mt-4 text-lg font-semibold">${price}<span className="ml-1 text-xs font-normal text-muted">one-time</span></p><Button size="sm" className="mt-4 w-full" onClick={() => onBuy(name)}>Add to checkout</Button></div></Card>)}</div></div>;
}
